...
#undef BBQ_DDCG_BINOP_MAKE
    ...
    // unions and alternatives). Every arm's success threads through CommitChoice,
    // which pops the Savepoint and chains to Lnext; the "exhausted" OnFailKont
    // pops it instead when the last arm fails too. Arms compile right-to-left so
    // each OnFailKont link knows the next arm's entry; the same link is both the
    // runtime Savepoint.on_fail_target (byte-fails) and the static
    // in_alt.next_arm (where-fails via on_false).
    bbq::cek::StaticKont*
    compile_choice(std::vector<std::string> names,
        ...
        // The last arm commits like every other one. It is ENTERED with the
        // Savepoint still on the stack — the link that dispatched to it has
        // pop_frame=false, and `exhausted` only runs if this arm fails too — so
        // succeeding without committing leaves that Savepoint standing over the
        // rest of the parse, and the enclosing construct never finishes.
        bbq::cek::StaticKont* last_entry =
            build_variant_arm(names[last], bodies[last], last, r, commit);
