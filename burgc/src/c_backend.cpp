...
public:
    void generate(std::ostream& out, const BurgAnalysis& a) override {
        ...
        // ── Context struct ──
        out << "/* ── Context ── */\n\n";
        out << "typedef struct BurgContext {\n";
        out << "    bbq_arena arena;\n";
        ...
        // ── Context lifecycle ──
        out << "static void burg_ctx_init(BurgContext* ctx) {\n";
        out << "    bbq_arena_init(&ctx->arena, 4096);\n";
        out << "    ctx->state_cache = bbq_htree_create();\n";
        out << "}\n\n";

        out << "static void burg_ctx_free(BurgContext* ctx) {\n";
        ...
        out << "}\n\n";
        out << "static void arena_reset(BurgContext* ctx) {\n";
        out << "    bbq_arena_reset(&ctx->arena);\n";
        out << "}\n\n";

        // ── Cache functions (delegate to htree) ──
        out << "/* ── State cache (graph path only) ── */\n\n";
        out << "static BurgState* burg_cache_lookup(uint32_t id, BurgContext* ctx) {\n";
        out << "    return (BurgState*)bbq_htree_search(ctx->state_cache, id);\n";
        out << "}\n\n";
        out << "static void burg_cache_store(uint32_t id, BurgState* state, BurgContext* ctx) {\n";
        ...
        // Rule lookup
        out << "/* ── Rule lookup ── */\n\n";
        out << "static BURG_UNUSED int burg_rule(BurgState* state, int goalnt) {\n";
        emit_rule_func_body(out, 1);
        out << "}\n\n";

        // NT name
        out << "/* ── Nonterminal names (for debugging) ── */\n\n";
        out << "static BURG_UNUSED const char* burg_nt_name(int nt) {\n";
...
void CBurgBackend::emit_label_body(std::ostream& out, int indent) {
    ...
    pad(out, indent); out << "BurgState* cached = burg_cache_lookup(id, ctx);\n";
...
void CBurgBackend::emit_rewrite_body(std::ostream& out, int indent) {
    ...
    pad(out, indent + 1); out << "BurgState* state = burg_label_tree(root, ctx);\n";
    if (a_->has_actions) {
    ...
    if (a_->has_actions) {
        ...
        pad(out, indent + 1); out << "int _i, _n = bbq_vec_len(rpo);\n";
        pad(out, indent + 1); out << "for (_i = 0; _i < _n; _i++) {\n";
        pad(out, indent + 2); out << "BurgState* s = burg_cache_lookup((uint32_t)(uintptr_t)BURG_NODE_ID(rpo[_i]), ctx);\n";
