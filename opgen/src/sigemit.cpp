...
void SigEmitter::build() {
    ...
    renumber_to_schema_order();
}

std::vector<int> SigEmitter::schema_final_order() const {
    ...
    for (size_t r = 0; r <= kVoid; r++)
        for (int id : by_result[r]) order.push_back(id);
    return order;
}

void SigEmitter::renumber_to_schema_order() {
    std::vector<int> old_of_new = schema_final_order();
    size_t nfinal = old_of_new.size();
    for (size_t i = 0; i < sigs_.size(); i++)
        if (!sigs_[i].is_final()) old_of_new.push_back((int)i);

    std::vector<int> new_of_old(sigs_.size());
    for (size_t n = 0; n < old_of_new.size(); n++) new_of_old[(size_t)old_of_new[n]] = (int)n;

    std::vector<Sig> ns(sigs_.size());
    for (size_t n = 0; n < old_of_new.size(); n++) ns[n] = sigs_[(size_t)old_of_new[n]];
    sigs_.swap(ns);
    sig_ids_.clear();
    for (size_t i = 0; i < sigs_.size(); i++) sig_ids_[sigs_[i]] = (int)i;

    for (auto& oi : ops_)
        for (auto& r : oi.resolved) r = new_of_old[(size_t)r];
    for (auto& c : carried_ids_) c = new_of_old[(size_t)c];
    final_ids_.clear();
    for (size_t i = 0; i < nfinal; i++) final_ids_.push_back((int)i);
}
...
void SigEmitter::emit_ttree_asdl(FILE* o) {
    fprintf(o, "// AUTO-GENERATED by opgen — do not edit.\n"
               ...
               "//\n"
               "// Constructor declaration order IS signature-id order — build() renumbers\n"
               "// the finals to this file's own bucket order — so a template emitting the\n"
               "// tag enum from this schema needs no id table beside it: dense enumeration\n"
               "// values equal jav_sigtab indices by construction, and PIN B-6 compares\n"
               "// the two ends name-for-name at every id.\n\n"
               "module %s_ttree {\n", prefix_.c_str());
    fprintf(o, "\n"
               "    // C carriers for the fields every node shares (the attributes below).\n"
               "    type u8    = \"uint8_t\"\n"
               "    type u32   = \"uint32_t\"\n"
               "    type bytep = \"const uint8_t*\"\n");
    // The fields every node carries whatever its constructor: the analysis'
    // spill-room quantity, the flat §5 ordinal, and the stitcher's byte position.
    // Stated here so the schema describes the WHOLE node, not just its tree shape
    // — the template renders these, it does not invent them.
    static const char* kAttrs = "attributes (u8 need, u32 seq, bytep pc)";
    std::vector<std::vector<int>> by_result((size_t)SCLASS_FINAL + 1);
    const size_t kVoid = SCLASS_FINAL;
    for (int id : final_ids_) {
        const Sig& s = sigs_[id];
